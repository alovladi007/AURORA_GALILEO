...
-- Shape matches services/data-service/src/database.py (the proto is
-- the schema of record). Composite PK includes the partition column
...
CREATE TABLE IF NOT EXISTS satellite_telemetry (
    ...
    -- satellite_id is a space-partitioning dimension (add_dimension
    -- below), so it must be part of every unique constraint too
    PRIMARY KEY (timestamp, satellite_id, id)
);
...
-- NOTE: no ll_to_earth GIST index here. earthdistance functions are
-- not schema-qualified internally, so TimescaleDB background workers
-- (materialization jobs run with a restricted search_path) fail to
-- inline them. The btree (latitude, longitude) index below serves
-- bounding-box queries.
CREATE INDEX IF NOT EXISTS idx_satellite_telemetry_latlon
    ON satellite_telemetry (latitude, longitude);

-- ============================================================================
-- Gravity Measurements Hypertable
-- ============================================================================

-- Shape matches services/data-service/src/database.py.
CREATE TABLE IF NOT EXISTS gravity_measurements (
    id                    BIGINT GENERATED BY DEFAULT AS IDENTITY,
    measurement_id        VARCHAR(128),
    satellite_id          VARCHAR(100) NOT NULL,
    timestamp             TIMESTAMP NOT NULL,
    latitude              DOUBLE PRECISION NOT NULL,
    longitude             DOUBLE PRECISION NOT NULL,
    altitude              DOUBLE PRECISION NOT NULL,
    gravity_value         DOUBLE PRECISION NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_gravity_quality ON gravity_measurements (quality_flag) WHERE quality_flag IN ('good', '1');
...
SELECT
    ...
    satellite_id,
    AVG(gravity_value) AS avg_anomaly,
    STDDEV(gravity_value) AS stddev_anomaly,
    AVG(uncertainty) AS avg_uncertainty,
    COUNT(*) AS measurement_count,
    COUNT(*) FILTER (WHERE quality_flag IN ('good', '1')) AS good_quality_count
