...
echo ""
echo "1. Train Model..."
curl -s -X POST http://localhost:28000/api/v1/models/train \
  -H "Content-Type: application/json" \
  -d '{"model_type": "pinn", "training_config": {"epochs": 100}}' | jq '.'

echo ""
echo "2. List Models..."
curl -s http://localhost:28000/api/v1/models | jq '.'
...
JOB_ID=$(curl -s -X POST http://localhost:28000/api/v1/inversions/run \
  -H "Content-Type: application/json" \
...
curl -s "http://localhost:28000/api/v1/inversions/${JOB_ID}/status" | jq '.'

echo ""
echo "3. List Inversions..."
curl -s http://localhost:28000/api/v1/inversions | jq '.'
...
PLAN_ID=$(curl -s -X POST http://localhost:28000/api/v1/missions/plans \
  -H "Content-Type: application/json" \
...
curl -s -X POST http://localhost:28000/api/v1/simulation/propagate \
  -H "Content-Type: application/json" \
...
curl -s -X POST http://localhost:28000/api/v1/control/maneuver \
  -H "Content-Type: application/json" \
...
curl -s -X POST http://localhost:28000/api/v1/data/gravity \
  -H "Content-Type: application/json" \
...
curl -s "http://localhost:28000/api/v1/data/gravity?satellite_ids=SAT-001&limit=5" | jq '.'
